...
// NOLINTNEXTLINE
TEST(Connect5GS, PlayMove) {
  auto board = SizedTensor<int8_t, Eigen::Sizes<2, HEIGHT, WIDTH>>{};
  board.setZero();

  auto x = Connect5GS{};
  auto y = Connect5GS{board, 0, 0};
  EXPECT_EQ(x, y);
  ...
  for (auto h = HEIGHT - 1; h >= 1; h -= 2) {
    x.play_move(3);
    board(0, h, 3) = 1;
    y = Connect5GS{board, 1, ++i};
    EXPECT_EQ(x, y);

    x.play_move(3);
    board(1, h - 1, 3) = 1;
    y = Connect5GS{board, 0, ++i};
    EXPECT_EQ(x, y);
  }
...
TEST(Connect5GS, WinState) {
  ...
  // Diagonal (down-right) win, player 0.
  board(0, 1, 1) = 1;
  board(0, 2, 2) = 1;
  board(0, 3, 3) = 1;
  board(0, 4, 4) = 1;
  ...
  // Anti-diagonal (down-left) win, player 1.
  board(1, 1, 5) = 1;
  board(1, 2, 4) = 1;
  board(1, 3, 3) = 1;
  board(1, 4, 2) = 1;
  ...
  // Full, alternating bottom row: no 5-in-a-row possible, board is full -> draw.
  board.setZero();
